...
export const KaraokePreview: React.FC<KaraokePreviewProps> = ({ videoRef }) => {
    ...
    const render = () => {
      ...
      if (styleConfig.bgType === 'color') {
        ...
        try {
          ...
          // Video frame not ready, fallback to dark blue
        ...
        // Fallback/Visuals (Interactive neon grid particles)
        ctx.fillStyle = '#05060c';
        ctx.fillRect(0, 0, canvas.width, canvas.height);
  ...
  }, [lines, currentTime, styleConfig, mediaType]);

  // Render method for Vietnamese Alternating 2-Lines
  const renderClassic2Line = (ctx: CanvasRenderingContext2D, width: number) => {
    // We need to decide which lines to display on:
    // Line 1 (Odd lines, y coordinate ~ 560px)
    // Line 2 (Even lines, y coordinate ~ 640px)
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          position: 'relative',
          background: '#000',
          boxShadow: '0 10px 40px rgba(0, 0, 0, 0.8), 0 0 20px rgba(139, 92, 246, 0.15)',
        }}
      >
        <canvas
          ref={canvasRef}
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>
              {mediaType === 'video' ? '📺 Gặp Video nền' : '🎵 Cảnh Âm thanh'}
            </span>
          </div>
