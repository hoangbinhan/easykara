...
export const CanvasPlayer: React.FC<CanvasPlayerProps> = ({ lines, mediaType, styleConfig }) => {
  ...
  const renderSubtitles = () => {
    ...
    if (!activeLine) {
      const upcoming = lines.find(
        (line) => line.startTime !== null && line.startTime > currentTime
      );
  ...
  return (
    <div ref={containerRef} className="absolute inset-0 w-full h-full overflow-hidden select-none">
      ...
      {styleConfig.bgType === 'color' && (
        <div className="absolute inset-0 z-0" style={{ backgroundColor: styleConfig.bgColor }} />
      )}

      {styleConfig.bgType === 'image' && styleConfig.bgImage && (
        <div className="absolute inset-0 z-0 select-none pointer-events-none">
          <img src={styleConfig.bgImage} className="w-full h-full object-cover" alt="Background" />
      ...
      <div className="absolute inset-0 z-20 w-full h-full pointer-events-none">
        {lines.length > 0 ? (
          styleConfig.layoutMode === 'classic-2line' ? (
            renderClassic2Line()
          ) : (
            renderSubtitles()
          )
