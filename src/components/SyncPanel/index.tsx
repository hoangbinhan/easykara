...
export const SyncPanel: React.FC<{ audioRef: React.RefObject<HTMLAudioElement | null> }> =
  React.memo(({ audioRef }) => {
    const { t } = useLanguage();
    const {
      playbackRate,
    ...
    return (
      ...
      >
        <div className="flex items-center justify-between">
          <div
            className={`flex items-center gap-2 ${isRecording ? 'text-neon-glow' : 'text-whiteout'}`}
          >
            <Keyboard size={14} />
        ...
        <div className="bg-blackout border border-graphite-light rounded-[4px] p-6 min-h-[130px] flex flex-col justify-center items-center gap-4 relative overflow-hidden w-full">
          {!mediaUrl ? (
            ...
            <div className="text-center">
              ...
              <p className="font-sans text-[11px] text-ash">{t('syncPanel.syncCompleteDetail')}</p>
            </div>
          ) : !isRecording ? (
            <div className="text-center flex flex-col gap-3.5">
              <p className="font-sans text-xs text-ash">{t('syncPanel.syncInstructions')}</p>
              <button
            ...
            <WordBubbles
              ...
              currentSyllableIndex={
                currentLine?.syllables.findIndex((s) => s.startTime === null) ?? -1
              }
