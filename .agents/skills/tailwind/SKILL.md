---
name: tailwind
description: Tailwind CSS v4.3.0 browser-runtime patterns for HyperFrames compositions. Use when scaffolding or editing projects created with `hyperframes init --tailwind`, writing Tailwind utility classes in composition HTML, adding CSS-first Tailwind v4 theme tokens, debugging v3 vs v4 syntax, or deciding when to compile Tailwind to CSS instead of using the browser runtime.
---

# Tailwind CSS for HyperFrames

HyperFrames `init --tailwind` uses the Tailwind browser runtime pinned to `@tailwindcss/browser@4.3.0`. Treat that as Tailwind v4, not v3.
...
- Pinned runtime: `@tailwindcss/browser@4.3.0`.
...
2. Confirm the script points to `@tailwindcss/browser@4.3.0`.
